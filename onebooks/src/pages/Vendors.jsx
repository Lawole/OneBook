...
const Vendors = () => {
  ...
  const openEdit = (v) => {
    setError('');
    setEditData({ _id: v.id, name: v.name, email: v.email, company_name: v.company_name || '', phone: v.phone || '', address: v.address || '' });
    setModal('edit');
  };
  ...
  const avatarColor = (name) => { const colors = ['#f59e0b','#10b981','#3b82f6','#8b5cf6','#ef4444','#ec4899']; return colors[name?.charCodeAt(0) % colors.length] || '#f59e0b'; };
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={v.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: avatarColor(v.name), color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>{initials(v.name)}</div>
                        <div>
                  ...
                  <tr><td colSpan="4"><div style={{ textAlign: 'center', padding: '50px 20px' }}>
                    ...
                    <div style={{ color: '#94a3b8', fontSize: 14 }}>Click "Add Vendor" to add your first vendor</div>
                  </div></td></tr>
                )}
              </tbody>
            </table>
      ...
      {(modal === 'create' || modal === 'edit') && (
        <div style={overlayStyle}>
          <div style={modalStyle}>
            ...
            <VendorForm initial={modal === 'edit' ? editData : blank} onSave={modal === 'edit' ? handleEdit : handleCreate} onCancel={closeModal} saving={saving} error={error} />
          </div>
        </div>
      )}
    </div>
